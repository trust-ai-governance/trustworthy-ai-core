...
def test_pattern_1_k_over_n_with_percent_or_ci():
    ...
    assert disclosure_hit("catch rate 25/28 = 89.3%") is not None
    assert disclosure_hit("FPR 0/100, 95% CI [0, 3.7%]") is not None
    assert (
...
def test_pattern_2_ci_value_but_not_a_threshold():
    ...
    assert disclosure_hit("its ci_low is 0.728 at n=28") is not None  # a VALUE
    assert (
...
def test_pattern_3_indicator_id_with_a_percentage():
    ...
    assert disclosure_hit("injection_catch_rate = 89%") is not None
    assert disclosure_hit("chain_integrity 100.0% (census)") is not None
    assert (
