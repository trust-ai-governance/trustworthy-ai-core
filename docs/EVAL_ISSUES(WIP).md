...
- **发现**:Platform 架构师查出 `policy_snapshot_version` 不是内容派生的 —— `上游侧`
  取的是配置里的静态串 `ruleset_version: "v1"`,**非内容派生**。⇒ 审计记录分不出跑的是哪版规则集。
- 🔴 **Core 侧核实后补一半**:这个字段 **Core 未消费该字段**(`grep policy_snapshot|ruleset_version
  treval/ tools/` → 0 命中;proto 里字段是有的)。**两侧各一半** —— 所以只改 Platform 一侧,
...
- 🔴 **motivating case(比设计缺陷本身更硬 —— 这是一次真实发生过的归因失败)**:
  ...
  fake-authority+disable-safety)。它落在 Platform 提交 `〔上游版本A〕` 里 —— 而那个提交的 subject 是
