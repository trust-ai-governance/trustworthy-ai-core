...
## 🔴 架构师验证纪律（2026-08-02 立 · 由三次连续误判逼出来的）

**五步（用户定版）：1 核 · 2 问 · 3 表述 · 4 验证 · 5 售前核。**

| 步 | 具体动作 | 它防的那次事故 |
|---|---|---|
| **1 核** | 断言任何阈值/规则/数字前，先 grep 到**权威落点**；🔴 **自己写过的同样要核** —— 一句话进了自己的文档，就会从"待核的主张"变成"可据以推理的前提" | 两个 `0.80` 撞号几轮无人发现；把"新增批次"的门槛套到存量全语料 |
| **2 问** | 不确定时**整理成带事实的编号问题**交出去，🔴 **不写"待确认"就交卷** | 件一/件三 归属曾被标成"⚠️ 待确认"直接上报 |
| **3 表述** | 引用必带可解析的东西：**数字带名字 · 章节带链接 · 阈值带 registry key**。裸 token 视为债 | 裸"80%"、裸"§6" |
| **4 验证** | 改完实跑复核 | —— 见下面三条子纪律 |
| **5 售前核** | 🔴 **统计 / 对外口径类结论，发出前过售前** | `n=28` 的置信区间问题是售前逮到的，不是我 |

### 🔴 「4 验证」的三条子纪律（每条对应一次真实误判）

1. **验断言本身，不验它的便宜近似。**
   *grep 仓内 = 0* 便宜；真正的断言是"产物存不存在"。*`'ci' in source`* 便宜；真正的断言是"这个指标填不填区间"。
2. **否定性证据只能证明"我找过的范围内没有"，不能证明"不存在"。**
   下结论前补一句：**"我找过的范围是什么？边界外是不是正是它该在的地方？"**
   （实例：冻结 artifact 按设计住仓外，路径就写在我引用的那份 spec 里。）
3. 🔴 **信任一个检查之前，先说出一个能让它返回 False 的输入。说不出来，这个检查就是空的。**
   （实例：`'ci' in src` —— `ci` 是 `decision`/`explicit` 的子串，任何输入都 True。）

> **为什么写在这里而不是某个 issue 里**：这三条不属于任何一个 issue，
> 它们是**架构师这个角色的债**。Implementer 的债由架构师 code review 抓；
> 🔴 **架构师的债今天只有人能抓**（PM 直觉 / Platform 顶回来 / 售前的统计论证 / 用户的一句"你糊涂了吧"）——
> **四次都是人，没有一道门。** 在有门之前，这五步就是替代品。
...
> **⚠️ 本表是 2026-07-19 的定稿快照，八项里六项已结。状态见下表最后一列；
> 真正要看的"下一步做什么"在本节末尾的 [🔴 统一次序](#-统一次序2026-07-30-重排--本节唯一权威)。**

| 序 | 项 | 为什么这个位置 | 规模 | **状态（2026-07-30 核对）** |
|---|---|---|---|---|
| 1 | **CI-1** 渲染守卫 CI 里 fail 非 skip | 原 P0,护旗舰守卫,且是 v0.2.0 门 | 小 | ✅ **已合并**（GATE-LASTMILE P1） |
| 2 | 🔴 **EV-PIN** 冻结评测 run(消除移动窗口) | PROV 的前置:对外数字须来自 pinned run,今天 collect 恒 `window=[0,0]`、无窗口参数 | 小 | ✅ **已合并**（`--window-from-ns/--window-to-ns` + `pinned` + `wal_segments` sha） |
| 3 | 🔴 **PROV** 数字 provenance 对账(**协作:Core 起头 + Platform 补齐**) | **唯一同时解锁待审稿白皮书 §5.2/§5.4 与 roadmap 对外数字**(9.2-B 硬门);依赖 EV-PIN | 小(Core)/ 跨仓 | ⏳ **Core 主体已闭环**（两个规范 run 冻结 + §5.0 仲裁表）；**余 [PROV-CLOSEOUT](issues/PROV-CLOSEOUT.md) 件一/二/三/五** ⇒ 见统一次序序 4 |
| 4 | **EV-FWD** standalone 标的抽象 | OSS 推广总开关,便宜、无生产 PII | 中 | ✅ **已合并**（`OpenAITarget` + `evidence_requirement` + 指标级 `availability`，schema v3） |
| 5 | **#4** 窗口键 → `generated_at_ns` | 潜伏 bug,顺手 | 小 | ⬜ **未做** —— 但 EV-PIN 落地后 `window` 不再恒 `[0,0]`，**触发条件已大幅收窄**（仅未 pin 的 active-only 跑仍可能撞键） |
| 6 | 🆕 **P3C-harness** P3-content spike 测量 harness | Platform 即将开 P3-content 跨仓依赖,spike 起跑时 Core 须能供 harness | 中 | ✅ **C1/C3/C4 已合并**（双侧门 · 延迟分布 · `content_class` 切片 · 稳定性/曲线指标 + verdict loader + I3 CLI）；🔒 **C2 厂商对照臂闸住**（无回复 + 数据条款未清） |
| 7 | **#6** 生产被动路径 | 大头(reader+租户隔离+PII 面);按有无活 POC 定序 | 中 | ⬜ **未做**（tiebreaker 未翻转：仍无活的生产报告 POC；`acme` 仅 49 条） |
| 8 | **P2 束** UI-3 证据下钻(网关)/#5 执行页/EV-R2/DX 一条命令/UI-6 趋势 | 产品完整性 | 中/大 | ⬜ **未做** |

### 第 3 轮定稿后另行交付（不在原表内，2026-07-19 → 07-30）

> 这些是当时表里没有、后来因裁定/事故/对外需求插进来的。**记在这里，免得下次 review 以为它们没做。**

| 项 | 内容 | 状态 |
|---|---|---|
| **R1 契约** | 报告级 `target_kind` + 派生 `evidence_basis`（[R1-TARGET-KIND-SCHEMA](issues/R1-TARGET-KIND-SCHEMA.md)） | ✅ 已合并 |
| **RUFF-SELECT** | 规则集显式声明（把"上游升级改规则"变成我们的决定） | ✅ 已合并 |
| **C4** | `eval_report` 标注标的/模式（消费 R1，不重定义） | ✅ 已合并 |
| **EV-PAIR + A + A2** | 配对评测（治理前/后 delta）· 裸模型出真数 · standalone 三条收口 | ✅ 已合并（含**门 7** 护栏信号 fail-closed） |
| 🔴 **披露收口 + 机械门** | 私有仓路径（18 处）+ 真实部署身份（6 处，含 3 处产品 docstring）中性化；`check_doc_disclosure.py` 加 `private_repo_path` / `internal_identity` 两条 error 规则；[DISCLOSURE_POLICY](DISCLOSURE_POLICY.md) 加 §1.2 方向纪律 + §6 流量口径政策 | ✅ 已合并 |

### 第 4 轮增补（2026-07-30 · 由对外材料 review 触发）

> 触发事件:宣讲材料 review 提出「拦截 89%,得逞率怎么会 0%」+「n=28 偏薄」。
> 追下去发现三件事各自独立、且都不在原 backlog 里。**次序按"先能说清、再说得准、最后说得多"**。

| 序 | 项 | 为什么这个位置 | 规模 |
|---|---|---|---|
| 9 | 🔴 **[EV-ATTRIB](issues/EV-ATTRIB.md)** 三格归因(机制 / 得逞 / **模型自觉**)+ 同分母分层 catch | **对外主张的正确性**:`injection_succeeded` 结构上分不开"治理拦下"与"模型没照做"⇒ 今天任何"0% 得逞"的归因都超出测量。**§3.1 的分层 catch 不改语料就能消掉"89% vs 0%"那个无效推理** | 小 |
| 10 | 🔴 **[EV-COVERAGE](issues/EV-COVERAGE.md)** 覆盖率四轴 + 语料门 + 扩语料 | **说得清 > 说得准**:实测发现 28 条注入用例的手法名**早在 `id` 里、只是没进 `attack_class`** ⇒ **E0 字段补录 0.5–1 人日,可声明手法覆盖 41→~113,不写一条新用例**。门必须早于扩语料 | E0+A~D 小(≈2.5–3 人日) / E3 扩语料 中(+1.5 人日 @ n=60) |
| 11 | **[EV-CAPCTRL](issues/EV-CAPCTRL.md)** 良性遵循对照臂 | 解锁跨模型比较(今天红线无条件);且是 EV-PAIR delta 缺的**误报面**那一半。其提交 C 与 EV-COVERAGE E3 是**同一批语料活** | 中 |
| 12 | **[PROV-CLOSEOUT](issues/PROV-CLOSEOUT.md)** 件一/二/三/五 | 零测量成本;件五给配对 delta 加**可引性判词**(流量口径 + Wilson 功效) | 小 |

🔴 **三个 issue 的语料活要合并下发**(否则同一批 yaml 被改三遍):
EV-COVERAGE 的 **E0 补录 / E3 扩 n / hold-out 切分** + EV-CAPCTRL 的 **良性孪生** +
EV-ATTRIB 登记的 **marker 用例扩充** —— **一次评审、一次冻结、一个新 `corpus_sha`**。

---

## 🔴 统一次序（2026-07-30 重排 · **本节唯一权威**）

> **上面两张表是历史记录，这张是"下一步做什么"。**
> 第 3 轮排序的三个协调判断**有两个已经失效**，所以不是接着原序号往下走：
>
> | 原判断 | 今天 |
> |---|---|
> | ① PROV(+EV-PIN) 排 EV-FWD 前 | ✅ **已执行完毕**，三项都落了 |
> | ② P3C-harness 可能抢占 EV-FWD | ✅ **两个都做完了**，不再冲突（余下的 C2 被外部闸住） |
> | ③ EV-FWD vs #6 看"有没有活 POC" | ⏸ **tiebreaker 未翻转** —— 仍无活 POC ⇒ #6 继续排后 |
>
> **新的排序原则（一句话）：先让已有的数字能被正确引用，再让它更准，最后才做新能力。**
> 理由：本轮四个新 issue 全部由**对外材料 review** 触发 —— 说明瓶颈已经从"测不出来"
> 转移到 **"测出来了但说不清、不敢引"**。

| 序 | 项 | 归类 | 为什么这个位置 | 规模 | 依赖 |
|---|---|---|---|---|---|
| **1** | 🔴 **[EV-COVERAGE](issues/EV-COVERAGE.md) E0**（字段补录 · **施工单 §4.2**） | 说得清 | **全程序性价比最高的一步**：0.5–1 人日、不写一条新用例，可声明手法覆盖 **41 → ~113**，`llm01` 单类占比 **57% → 3.6%**。且**必须早于立门**（先修到合规再立门，比"会红的门 + 长期豁免"干净） | **0.5–1 人日** | 无 |
| **2** | 🔴 **[EV-ATTRIB](issues/EV-ATTRIB.md)** 三格归因 + 同分母分层 catch | 说得清 | **对外主张的正确性**：`injection_succeeded` 结构上分不开"治理拦下"与"模型没照做"⇒ 任何"0% 得逞"的归因都超出测量。**§3.1 的分层 catch 不改语料就消掉「89% vs 0%」那个无效推理** | 小 | 无 |
| **3** | **[EV-COVERAGE](issues/EV-COVERAGE.md) A–D**（四轴报表 + 语料门 + hold-out · **施工单 §4.3**） | 说得清 | 把覆盖率从形容词变成**可算、可声明、随 `corpus_sha` 同框**的四个数；门把"n 涨必须带来覆盖涨"变成机械约束 | 小（≈2 人日） | 序 1 |
| **4** | **[PROV-CLOSEOUT](issues/PROV-CLOSEOUT.md)** 件一/二/三/五 | 说得清 | 零测量成本；件五给配对 delta 加**可引性判词**（流量口径 + Wilson 功效）—— 现在任何人拿 `pair` 产物做胶片都不会被挡一下 | 小 | 无 |
| **5** | **[EV-CAPCTRL](issues/EV-CAPCTRL.md)** 良性遵循对照臂 | 新能力 | 解锁跨模型比较（今天红线**无条件**生效）；且是 EV-PAIR delta 缺的**误报面**那一半 | 中 | 序 1（同批语料） |
| **6** | **[EV-COVERAGE](issues/EV-COVERAGE.md) E3** 扩语料 @ n≈60 | 说得准 | 89% 的半宽 **±11.7 → ±7.7**。🔴 **收益递减**：100→150 只再收 1.2 点 ⇒ **不以 n 为目标，以"每手法 1–3 例"为目标** | 中（+1.5 人日） | 序 3（门先立）· 与序 5 **合批** |
| **7** | **RULEPIN-CORE** 数字自带规则集指纹 | 新能力 | measured>attested 的最后一块：已能证"跑的哪批 WAL"，还差"跑的哪版规则" | 小 | 上游半边 |
| **8** | **#4** 窗口键 → `generated_at_ns` | 收尾 | 潜伏 bug；EV-PIN 后触发面已收窄，顺手做 | 小 | 无 |
| **9** | **#6** 生产被动路径 | 新能力 | tiebreaker 未翻转（无活 POC，`acme` 仅 49 条） | 中 | 活 POC |
| **10** | **P2 束** UI-3 / #5 / EV-R2 / DX / UI-6 | 产品 | 产品完整性 | 中/大 | — |
| 🔒 | **P3C-harness C2** 厂商对照臂 | 外部闸 | 候选无回复 + **数据条款未清（别在准入前送数据）** | 小 | 外部 |
| 🔒 | **(b) 档代表性语料 + pinned 配对跑** | 外部闸 | 等 lead/售前裁定"是否发布 assumed-mix 图" | 中 | 业务裁定 |

### 下发就绪度（2026-07-31 · **全部裁定已落定，无 blocker**）

| 序 | 施工单 | 可下发？ | 裁定状态 |
|---|---|---|---|
| **1** | [EV-COVERAGE](issues/EV-COVERAGE.md) §4.2（E0） | ✅ **可下发** | D1/D4 PM 已签。**余 D4 的 3 个具体判**（`canary_*`×2 / `neutralize` 去前缀重名 / 跨语料同名）—— **防御域的活**，与 EV-CAPCTRL P5 同一批人；**不阻塞开工**（先补录无争议的，3 条挂起待判） |
| **2** | [EV-ATTRIB](issues/EV-ATTRIB.md) 全文 | ✅ **可下发** | 无待裁 |
| **3** | [EV-COVERAGE](issues/EV-COVERAGE.md) §4.3（A–D） | ✅ **可下发** | D2/D3 PM 已签，无余留 |
| **4** | [PROV-CLOSEOUT](issues/PROV-CLOSEOUT.md) 件一/二/三/五 | ✅ **可下发** | 无待裁 |
| **5** | [EV-CAPCTRL](issues/EV-CAPCTRL.md) | ✅ **A/B/D 可下发**；C 随语料批 | **P1–P5 全部落定（2026-07-31）**。🔴 **P4 签的同时带来一条新代码要求**：提交 D 增加「`pair` 结构上吐不出单独 delta」的**焊接**（§5.1-②）；良性臂**缺失**时 `citable:false`（§5.1.1，堵"没跑良性"这个后门） |
| **6** | [EV-COVERAGE E3 · 序6](issues/EV-COVERAGE-E3.md) | ✅ **闸已解除（2026-08-11，上游核对：无依赖，可开工）**；设计定稿见左链。🔴 **三条臂**：攻击 28→150 · 可观测 8→≥40 · **良性 19→≥73** —— 只扩攻击侧 L2 仍 unmet、等级不动。原暂缓记录： | **不接受 ③=67%、不开序 6，待 Platform Architect 出设计/技术理由。** 🔴 **议题已重述**：目标该是**可观测 n ≥ 40**（归因四格的精度 ±26 → ±13 点），**不是语料总量 n=60**；③ 的百分比是副产品不是目标。**不阻塞序 1–5 的提交** |

🔴 **跨单联动（下发时必须一起交代，否则会被拆散做）**：

1. **序 4 的 `citable_blockers` 与序 5 的 §5.1.1 是同一个字段** —— 序 4 先建三条阻塞（流量口径 / 功效 / **良性臂缺失**），序 5 的提交 D 再往里填值。**序 4 就要把第三条留好位置。**
2. **序 1 的 `attack_technique` 与序 3 的门是同一根轴** —— 门作用在 `attack_technique`（高基数），**永不作用在 `attack_class`**（两值，跑率型门必永远 FAIL）。
3. **序 5 提交 C 的良性孪生 + 序 6 的扩 n + hold-out 切分 = 一批语料活** —— **一次评审、一次冻结、一个新 `corpus_sha`**。

### 给 PM / 售前的三句话

1. **序 1–4 全是"让现有数字能被正确引用"**，合计 ≈ **4–5 人日**，**不产生任何新测量**。
   做完之后：覆盖率可明示、`89% vs 0%` 的质疑在产物层面消失、配对 delta 自带可引性判词。
2. **序 5–6 是同一批语料活，必须合批下发**（EV-CAPCTRL 良性孪生 + EV-COVERAGE 扩 n + hold-out
   + marker 扩充）—— **一次评审、一次冻结、一个新 `corpus_sha`**。分开做会让 sha 变三次、对外口径跟着乱三回。
3. 🔴 **在序 1–4 落地前，宣讲材料里的输出侧 0% 一律按已有先例处理**（撤或降级为口径下界）；
   **可用且机制归因的数是 `injection_catch_rate` 89% (n=28, 95% CI [72.8%, 96.3%])** —— 它纯读决策记录，不含模型自觉。
...
**一句话**:让 `treval` 能对**真实生产租户的流量**出报告 —— 而不是像今天这样,
只对**我们自己发的探针**(评测租户)出报告。

- **今天是什么**:两条管线的样本**都是我们自己造的**。主动侧发合成语料;被动侧
  ([collect.py:13](../treval/cli/collect.py#L13) "Production-scoped passive reads land later")
  读的也是**评测租户自己那份 WAL**,即刚才那批探针的记录。
  ⇒ 🔴 **仓里今天不存在任何一条真实用户的请求。**
- **#6 改什么**:被动读**生产租户**的 WAL,挂**被动侧接缝** `AuditEvidenceReader`
  (第三个 reader),**非** EV-FWD 的 `ProbeResult` 缝。`collect` 去掉 `--gateway` 硬依赖。
- **范围**:生产 WAL 被动读 + **租户隔离** + 🔴 **PII 面独立评估**(比 EV-W1 只读服务的 PII 面大得多)。
- **设计**:上游"标的抽象 + 输出侧同轴对比"设计(**私有,不公开**)有一节讲被动侧接缝;
  公开侧的落点是 `treval/protocols.py::AuditEvidenceReader`。
- 🔴 **阻塞项已换（2026-08-04,上游逐条实码核实后）**:原 tiebreaker "有没有活的生产 POC" **作废**。
  上游确认生产 WAL 是 **A 档(正文级)**:请求正文**逐字上盘、零脱敏、无开关可降档**;
  **无按主体删除机制**(哈希链使选择性擦除被设计成不可能,最粗可用单位是整段,而**一段交织多租户**);
  **存储层尚无租户隔离** —— #6 分工里"上游提供**租户隔离的**生产 WAL 访问"这一半**今天不存在**。
  ⇒ **新阻塞项 = 上游有没有可结构隔离、可按主体删除的证据面。**
  就算明天有活 POC,Core 拿到的也是一份**多租户交织、正文级、不可按主体删除**的日志。
  详见 [UI-3 §14](issues/UI-3.md)。
- ✅ **一个好消息**:评测租户与生产**走同一条代码路径**,字段填充度一致
  ⇒ `chain_integrity` / `unclosed_loop_rate` / `duration_p99` 在生产上**算得出**。
- 🔴 **Core 侧已裁**:**Core 不持有生产个人数据副本**(案级库只收合成语料契约,
  且这条**结构性成立** —— 生产请求产生不了 `case_id` 行);
  **UI-3b 不得复用案级库,也不得新建"生产逐请求库"**。
- 🔴 **它为什么是 [UI-3b](issues/UI-3.md) 的前置**:`request_id` 今天指向的是**我们自己的合成探针**;
  #6 落地的那一刻,同一个字段开始指向**真实用户的请求**。
...
- 🔴 **范围已划死**:**不引入用户表/口令/OIDC/角色矩阵/服务端会话** ——
  本单**不引入任何新的身份概念**,只给已有那把共享密钥补上**名字、期限、痕迹**。
- **四件**:①密钥有身份(`label`/`scope`/`expires_at`,页面显示"以 X 身份查看")·
  ②生命周期(到期即失效 · 删行即撤销 · 改文件热生效)· 🔴 ③**访问留痕** ·
  ④失败可见但不可枚举(未知/过期/撤销**同一句文案 + 恒定延迟**)。
- 🔴 **③ 是本单主体**:一份 `operator_only` 的绕过地图,**谁看过必须有记录**。
  且它**必须自述不是审计链**(能写的人也能删)—— 同 `cases verify` 的作用域声明纪律。
- **登记**:密钥**发放**(谁生成、怎么交给审计员)本单不解;触发 = 出现第三方审计员自助取密钥的需求。

### 🆕 GATE-EGRESS — 内容出口门 + 工件自述级别（✅ **已实现,本轮**）

- **设计定稿**:[docs/issues/GATE-EGRESS.md](issues/GATE-EGRESS.md);落地:`tools/check_egress.py`(CI 第 4 门,哨兵端到端 + 名字表)· `tools/wal_dump.py` 自述头部(`--decode/--join`,stderr)。
- **触因**:上游确认审计日志是 **A 档(正文级、零脱敏、不可降档、无按主体删除)**
  ⇒ 两件此前只是理论的事同时变活。
- **件一 内容出口回归**:🔴 `AuditEvidence.record` 就是整条解码记录 ⇒ **每个被动指标手里都握着完整 prompt**;
  挡住它进产物的**不是机制,是"到今天没人写那行代码"**。**按字段名 + 按哨兵值双查**,CI 拦。
- **件二 `wal_dump` 自述级别**:`--decode/--join` 打正文是本职(权限门 = 能不能读到 WAL 文件),
  但**输出不声明自己是什么级别** —— 这一家里只有它不自述。🔴 **不加 `--tenant` 过滤器**
  (过滤器不是边界,却看起来像 —— 同 GATE-CONSISTENCY 件一的 `[GATED]` 裁定)。
- **与 UI-3 无依赖,可并行。** 三层关系:**B-3 管库 · 件一管工件 · 件二管剪贴板**。

### P2 束（产品完整性,数据/裁定到位后）

- **UI-3** 证据下钻 —— 🔴 **已拆两件**(设计定稿:[docs/issues/UI-3.md](issues/UI-3.md)):
  - **UI-3a**(✅ **已实现,本轮**):**用例级**复算服务(EV-R2 Tier-0)。合成语料 ⇒ **PII 面无新增**;
    形状 = 独立进程 + 单租户绑定库 + ingest 披露门 + 复算面。**无外部依赖。**
    落地:`treval/case_contract.py`(纯读取半边)· `treval/case_store.py` + `treval cases store`(ingest 门)·
    `treval/web/cases_app.py` + `cases_auth.py`(凭据作用域)· 契约 schema v3 加 `tenant_id`。
  - **UI-3b**(不做,登记):**请求级** `/evidence`(重开 EV-W1 D3 撤销的那条)。真实流量 ⇒ **真 PII 面**,
    **依赖 #6 生产被动路径**,落地时须重做威胁模型(与 EV-R2 P4 同一件事)。
