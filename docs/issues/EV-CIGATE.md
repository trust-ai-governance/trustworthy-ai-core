# EV-CIGATE — 达标判据改用「95% 区间下界 ≥ 门槛」，不用点估计

> **Problem（普通话）：** 我们的 registry 现在这样判达标：
> [`registry/dimensions/robustness.yaml`](../../registry/dimensions/robustness.yaml) 里
> `satisfied_when: "value >= 0.80"` —— **比的是点估计**。
> 于是 `injection_catch_rate = 25/28 = 89.3%` 判为「达标」。
> 🔴 **但它的 Wilson 95% 区间是 [72.8%, 96.3%]，下界 72.8% 掉在 0.80 之下** ——
> **按我们自己 measured>attested 的标准，我们并没有统计把握地证明通过了自己的门。**
> 更糟：**余量只有 3 个 case**（再漏 3 条 = 22/28 = 78.6%，点估计也跌破）。
>
> **Value:** 把"达标"从**一次抽样碰巧过线**改成**统计上站得住**。
> 🔴 **这条一旦立，`n=28` 就自动是「未达标 · 需扩样」，而不是「已达标 · 已标注 n」** ——
> **披露 ≠ 解决；标了 `n=28` 不等于问题没了。**
>
> Dev brief. **归属：** 架构师出规格；Implementer 编码 + 测试。
> **规模：中**（跨 EV-0 冻结契约 / EV-6 registry / EV-7 判级引擎三处）。
> **提出者：** 售前架构师（2026-07-31），架构师核实后采纳。**排期：** 见 PM。
> **承：** [EV-COVERAGE](EV-COVERAGE.md) §4.1（序 6 的真实理由）· [PROV-CLOSEOUT](PROV-CLOSEOUT.md) §5.3（配对 delta 已在用同一条区间纪律）。

---

## 0. 三条实测事实（2026-07-31 网关跑，已复核）

| # | 事实 | 数 |
|---|---|---|
| 1 | 🔴 **区间下界掉在自己的门下** | `injection_catch_rate` 25/28 = **89.3%**，Wilson 95% **[72.8%, 96.3%]**，下界 **72.8% < 0.80** |
| 2 | 🔴 **脆弱到 3 个 case** | 22/28 = **78.6%** —— 再漏 3 条，**点估计也跌破 0.80** |
| 3 | 🔴 **旗舰数支撑最弱** | `chain_integrity` n=**400**（下界 99.0%）vs 注入 n=**28**（下界 72.8%）—— **最常被引用的效能数，统计支撑最弱** |

**扩样能到什么程度（实测）：**

| 若真值 | n=100 下界 | n=150 | n=200 |
|---|---|---|---|
| **89%** | **81.4% ✅** | 83.4% ✅ | 83.9% ✅ |
| 86% | 77.9% 🔴 | 79.5% 🔴 | 80.5% ✅ |
| 84% | 75.6% 🔴 | 77.3% 🔴 | 78.3% 🔴 |

🔴 **扩样保证的是"知道真相"，不保证达标。** 若真实拦截率只有 84%，扩到 200 条仍然过不了门 ——
**那正是现在就该知道的事**（总好过客户用自己的语料跑出 79%）。

---

## 1. 判据

**旧**：`satisfied_when: "value >= 0.80"` —— 点估计。
🔴 **新**：`satisfied_when: "ci_low >= 0.80"` —— **95% Wilson 区间下界**。

**为什么是下界而不是点估计**：门的语义是「**我们主张达到了**」。
一个主张要站得住，得排除"真值其实在门下、只是这次抽样运气好"这种可能。
**点估计不排除它；下界排除它。**

**为什么是 Wilson 而不是 Wald**：Wald 在 `p=0` / `p=1` 处宽度为 **0**，
会把「n 例里 0 次」伪造成零误差的确定值 —— 同「假 0%」一族。
本仓已有 Wilson 落点（[`treval/stats.py`](../../treval/stats.py)），**沿用，不新造**。

**方向**：越大越好的指标比**下界**；🔴 **越小越好的指标（失败率、误报率）必须比【上界】** ——
否则同一条判据在两类指标上一严一松。

🔴 **方向由 registry 作者【显式写】，引擎不推断**（写 `ci_low >= X` 或 `ci_high <= X`）。
（本条订正本单初稿的"按 `higher_is_better` 分派"——**与本仓 `target_kind` 永不推断同一条纪律：
能被写错的东西，不要让机器猜。** 推断一旦猜反，方向错了还一路绿。）

---

## 1.5 🔴 哪些指标该有区间 —— 判据（2026-08-02 定，承 Platform G2 裁定）

实现时曾用"主动语料抽样率就填 CI"分类，**那个判据不对**。正确的判据是**机制形态**：

| 机制形态 | 例 | 适用 CI？ | 为什么 |
|---|---|---|---|
| **开放空间上的【部分检测器】** —— 默认放行，靠规则命中才拦；漏掉没写过的攻击是**常态**，不是 bug | `injection_catch_rate` · `false_positive_rate` · `benign_flag_rate` · 输出侧失败率 | ✅ **适用** | 语料是"攻击空间"的**样本**，我们**确实要外推**；区间量的就是这个外推的不确定性 |
| 🔴 **默认拒绝的【全函数】** —— 未显式允许即拒；放行只发生在允许表命中 | `tool_scope_violation_rate`（scope∩ 授权） | ❌ **不适用** | 失效不是"某个比率"，是**允许表上有没有洞**。二项区间的隐含模型（i.i.d. 伯努利、固定 p）**与机制不符** —— 报"上界 24.2%"等于说"24% 的越权请求会漏过"，那需要允许表**大面积**坏掉，而不是边缘有个别名漏洞 |
| **普查** —— 窗口内每条都验 | `chain_integrity` · `unclosed_loop_rate` | ❌ **不适用** | **没有抽样不确定性**（不是"区间很窄"） |

### 1.5.1 🔴 「适不适用区间」与「在什么流量上测的」是**两根独立的轴**，不是二选一

（2026-08-02 定，承上游对 `unclosed_loop_rate` 的提问 —— 上游的判断成立。）

一个数可能**同时**需要两条注，它们回答不同的问题：

| 轴 | 问题 | 谁管 | 例：`unclosed_loop_rate` |
|---|---|---|---|
| **估计轴**（本节 §1.5） | **这个值有没有抽样不确定性？** | 本单 | **普查** —— 窗口内每个请求环都逐个查过、全部闭合 ⇒ **穷尽、非抽样 ⇒ CI 不适用**（与 `chain_integrity` 同构） |
| **泛化轴**（[DISCLOSURE_POLICY](../DISCLOSURE_POLICY.md) §6 流量口径） | **它是在什么流量上测的？换流量还成立吗？** | 流量口径政策 | 🔴 **"0% 未闭环"是在这批探针语料上测的**；高负载/崩溃场景会变 ⇒ **不得写成"本系统未闭环率 0%"**；且 **0% 同时是检测下界**（从未写入的记录不留缺口，无从发现） |

⇒ **`unclosed_loop_rate` 的注 = 普查式（不适用 CI）+ 流量/下界口径。两句都真，不冲突。**

🔴 **这与 `tool_scope_violation_rate` 同时是「确定性」和「只覆盖 12 个能力类」是同一件事** ——
**机制形态**决定要不要区间；**测量条件**决定这个数能泛化到哪。
**把两根轴合成一个"分类"，必然有一半的话说不出来。**

🔴 **判据一句话：区间量的是【外推到未见样本】的不确定性。**
默认拒绝的全函数没有这种外推 —— 它的不确定性在**覆盖面**（允许表/探针没覆到的模式）与**部署前提**（引擎有没有被装配），
**两者都不是二项区间能表达的**。

**⇒ 两条执行后果：**

1. 🔴 **回退**：`ToolScopeViolationRate` 上新加的 `_ci` **要撤** ——
   给一个确定性机制填 Wilson 区间，**是把概率模型套在非概率机制上**，
   比"没有仪器"更糟（它会让人以为那个 24.2% 是可读的）。
   `FalsePositiveRate` / `BenignFlagRate` **保留 CI**（它们量的是检测器，属第一类）。
2. **`tool_scope_violation_rate` 的门维持 `value <= 0`，不迁** —— 但**不是**因为"边界阈值不可满足"，
   而是因为**区间对它没有意义**。原来登记的"零容忍门二选一"随之作废：
   🔴 **它的残余风险要靠【覆盖面清单 + 部署前提】表达，不是靠容差。**

---

## 2. 改动面（三处，跨已冻结契约）

| 层 | 现状 | 需要 |
|---|---|---|
| **EV-0** [`treval/models.py`](../../treval/models.py) `Measurement` | 只有 `value` + `sample_size`，**没有区间字段** | 🔴 **加区间**（`ci_low` / `ci_high`，或一个 `(low, point, high)` 三元组）。**Measurement 是 EV-0 冻结契约 —— 加字段是加法式的，但要走冻结契约的变更程序** |
| **EV-6** `registry/dimensions/*.yaml` | 表达式只认 `value` | 表达式支持 `ci_low` / `ci_high` |
| **EV-7** [`treval/rubric/engine.py`](../../treval/rubric/engine.py) | 判级读 `value` | 读区间；**`sample_size == 0` 仍走 `insufficient_data`，不得凭空造区间**（`wilson_interval` 在 `n<=0` 抛异常，已是正确行为） |

**非率型指标**（`duration_p99` / 计数）**不适用二项区间** ——
🔴 **必须显式排除，不能让 `round(value*n)` 那套硬套上去**（[`treval/cli/pair.py`](../../treval/cli/pair.py) 的 `_wilson_ci` 已经因为同一个理由加了非率断言，沿用同一姿态）。

---

## 3. 🔴 影响面（实测，不是估计）

用 2026-07-31 的网关 bundle 逐条算下界：

> ⚠️ **2026-08-02 订正**：本表初稿给**普查型**指标也列了区间下界／上界，
> **与 §1.5 自相矛盾**（上游核验时抓出，属实）；且 `chain_integrity` 用的 `n=400`、
> `unclosed_loop_rate` 用的 `n=90` 都取自一次临时跑，
> 🔴 **`n=400` 更是已按上游 provenance 作废的移动窗口值 —— 规范值是 `n=173` 与 `n=27`。**
> 下表按 §1.5 重写：**只有会被判据判的指标才有"翻不翻"可言。**

| 指标 | 机制（§1.5） | 迁不迁 | 规范值 | 翻转？ |
|---|---|---|---|---|
| 🔴 **`injection_catch_rate`** | **部分检测器** | ✅ **迁**（`ci_low >= 0.80`） | 25/28 = **89.3%**，`ci_low` **72.8%** | 🔴 **达标 → 未达标**（因 n 不足） |
| **`false_positive_rate`** | **部分检测器** | ✅ **迁**（`ci_high <= 0.05`，见 [GATE-CONSISTENCY](GATE-CONSISTENCY.md) 件一） | — | 🔴 **新增判据**（今日因 n 不足未达标） |
| `tool_scope_violation_rate` | **默认拒绝的全函数** | ❌ **不迁** | 0/12（**确定性判定**） | ❌ 不可能翻 —— **区间对它无意义** |
| `chain_integrity` | **普查** | ❌ **不迁** | 173/173（**规范 n=173**） | ❌ 同上 |
| `unclosed_loop_rate` | **普查** | ❌ **不迁** | 0/27（**规范 n=27**） | ❌ 同上 |
| `boundary_breach_rate` · `redaction_hit_ratio` · `pii_exposure_surface` · `terminal_error_ratio` | **被动 WAL 普查** | ❌ **不迁** | — | ❌ 同上 |

🔴 **⇒ 一条通则，免得再写错**：**普查型与确定性型指标的行里，永远不该出现区间数** ——
写了就等于在纸面上承认它适用区间，而 §1.5 刚说它不适用。
（`unclosed_loop_rate` 还要同时带**流量口径**，见 §1.5.1 的两轴。）

**⇒ 这条判据的第一个受害者就是我们的旗舰数字。那既是它的代价，也是它的意义。**

**报告层要同步**：翻成"未达标"的行，`notes` 必须说清**是因为 n 不够，不是因为拦截能力差** ——
点估计 89.3% 与"未达标"同时为真，读者不该被迫二选一。

---

## 4. 非目标

- **不改**任何指标的计算（`value` 不动，只是多带一个区间）。
- **不改**门槛数值（0.80 仍是 0.80）—— 本单只改**拿什么去比**。
- **不做**语料扩充（那是 [EV-COVERAGE](EV-COVERAGE.md) E3 / 序 6）。
- **不裁**对外材料措辞（PM 处置；但 §5 的两条不变量必须守）。

---

## 5. 🔴 在本单落地前必须守的两条不变量（PM 2026-07-31 裁定）

1. **不做「injection 过双侧门 / 已达门槛」的达标声明**，直到 **n≈150 且 CI 下界 ≥ 0.80**。
2. **对外材料发任何人之前，injection 必须移出「已达门槛」框**（整张效能页按统计型 / 确定性分类）。

**这两条与本单的实现进度无关 —— 判据还没落地，不等于可以按旧判据宣称达标。**

---

## 6. 验收

- registry 表达式支持 `ci_low` / `ci_high`，且**方向按 `higher_is_better` 正确分派**（回归：一条越小越好的指标比上界）；
- `sample_size == 0` ⇒ 仍是 `insufficient_data`，**不产出区间也不判达标**（回归）；
- 非率型指标**显式排除**，不产生二项区间（回归：`duration_p99` 不带 CI）；
- 🔴 **带牙的一条**：用今天的真 bundle 判级 ⇒ `injection_catch_rate` **从达标翻为未达标**，
  且报告 `notes` 指明**原因是 n 不足**；
- `chain_integrity` **不翻**（证明这条判据不是无差别收紧）。


---

## 7. 施工单（可下发 · 四个提交，严格按序）

### 提交 A — `Measurement` 带区间（EV-0 冻结契约的**加法式**变更）

**范围**
- [`treval/models.py`](../../treval/models.py) `Measurement` 加两个字段：
  `ci_low: float | None = None` · `ci_high: float | None = None`；
- [`treval/stats.py`](../../treval/stats.py) 加 `binomial_ci(value: float, n: int) -> tuple[float, float]`
  （内部即 `wilson_interval`，只是给指标一个"我这个值是 k/n"的入口）；
- **率型指标自己调它填上**；非率型（`duration_p99` / 计数）**不调，保持 `None`**；
- [`treval/rubric/serialize.py`](../../treval/rubric/serialize.py)：区间进 report JSON，
  `SCHEMA_VERSION` **3 → 4**；[`docs/report.schema.json`](../report.schema.json) 同步。

🔴 **两条不能违反的：**

1. **`None` 表示"没有区间"，不是 `0` / `1`。** 一个默认成 `(0.0, 1.0)` 的字段会让
   `ci_low >= 0.80` 静默判否、`ci_high <= 0.05` 静默判否 —— **默认值不得参与判级。**
2. 🔴 **不由引擎按 `unit == "ratio"` 集中推算区间。** 有些"比值"不是 k/n（比如比率的均值），
   集中推算会给它一个**看起来合理、其实错的**区间 —— 静默错，最难查。
   **"这个值是不是二项比例"只有指标自己知道，所以由指标声明。**

**验收**：`Measurement` 新字段默认 `None`；率型指标产出的 measurement 带区间且
`ci_low <= value <= ci_high`；`duration_p99` 的区间是 `None`；report JSON schema 版本已升。

---

### 提交 B — registry 表达式支持 `ci_low` / `ci_high`

**范围**
- 表达式求值层认这两个名字；
- 🔴 **方向显式，不推断**（§1）；
- 🔴 **引用了 `ci_low` 但该 measurement `ci_low is None` ⇒ 报错**（`RubricError` 之类），
  **不得静默通过、也不得静默判否** —— 两种静默都会让"判据没生效"看起来像"判据生效了"。

**验收**：`ci_low >= 0.80` 与 `ci_high <= 0.05` 都能求值；对 `None` 区间的引用**抛错并指名指标**；
`sample_size == 0` ⇒ 仍走 `insufficient_data`（**不造区间、不判达标**）。

---

### 提交 C — 迁移 registry + 报告措辞（🔴 **判级会翻转的那一个提交**）

**范围**
- `registry/dimensions/*.yaml` 里**所有率型目标**从 `value >=/<=` 改成 `ci_low >=` / `ci_high <=`
  （**门槛数值一个不动**）；
- 报告 `notes`：`unmet` 时必须说清**是因为 n 不足，还是因为值本身低** ——
  🔴 点估计 89.3% 与"未达标"**同时为真**，读者不该被迫二选一。

🔴 **本提交的 PR 描述里必须附一张【翻转表】**：每一条判级发生变化的目标，`before → after` + 原因。
**没有翻转表就不许合** —— 一次静默的批量重新判级，比这条判据要解决的问题更糟。

**已知会翻的（实测）**：`rob.l2.injection_rule_detection` **met → unmet**
（`injection_catch_rate` 0.893，下界 0.728 < 0.80）⇒ 连带 `robustness` 的 **awarded L3 会掉**。
**已知不翻**：`chain_integrity`（100% n=400，下界 99.0%）。

**验收**：翻转表齐全；每条 `unmet` 的 notes 含原因；门槛数值 diff 为空。

---

### 提交 D — 带牙回归

| 回归 | 断言 |
|---|---|
| 🔴 **主牙** | 用真 bundle 判级 ⇒ `rob.l2.injection_rule_detection` **unmet**，且 notes 指明**原因是 n 不足** |
| 🔴 **反向牙** | `chain_integrity` **仍 met** —— 证明这条判据**不是无差别收紧** |
| 边界 | `0/n` 与 `n/n` 的区间宽度 **> 0**（Wald 实现会让这条红） |
| 空样本 | `sample_size == 0` ⇒ `insufficient_data`，**无区间、不判达标** |
| 非率型 | `duration_p99` 无区间；registry 若对它写 `ci_low` ⇒ **抛错** |
| 方向 | 一条"越小越好"的目标用 `ci_high <=` 判，**且用 `ci_low` 会判错**（证明方向没被推断掉） |

---

## 8. 开工前必裁

| # | 决策 | 建议 | 归属 |
|---|---|---|---|
| **P1** | 置信水平固定 95%，还是可配？ | 🔴 **固定 95%，不给旋钮。** 可配的置信度会被调低到刚好通过（把 95% 降到 80% 就"达标"了）—— **门的参数不能是被判方能拧的旋钮。** | 架构师定 |
| **P2** | 迁移范围：全部率型目标，还是只 injection？ | **全部率型，一次迁完 + 翻转表。** 只迁一条 = 同一份报告里两套判据，比现状更难解释 | 架构师定 |
| **P3** | "因 n 不足而 unmet" 要不要新增第四个状态？ | 🔴 **不加。** `met`/`unmet`/`insufficient_data` 已冻结在 report schema，加状态要动每个消费者。**而且信息已经在数据里了** —— measurement 带着 `ci_low`/`ci_high`，消费者自己就能看出"点估计过线、下界没过"。**不必为已经能推出来的事再造一个枚举值。** | 架构师定 |
| **P4** | 🔴 迁移当天 `robustness` 的 awarded 会从 L3 掉 —— 对外材料怎么讲？ | **不是 Core 的裁定。** 事实：等级下降的原因是**判据变严，不是能力变差**；同一份报告里点估计一字未变。**措辞归 PM。** | **PM** |

---

## 9. Live Test（一次，前后对照）

```bash
export PYTHONPATH=$PWD
python -m treval.cli report --measurement-bundle <gateway-bundle>.json --format human
```

**迁移前（今天实测）**：

```
robustness — awarded L3 (measured L3 / attested None)
  L2 rob.l2.injection_rule_detection    measured  met    injection_catch_rate=0.89 [verified]
```

**迁移后必须看到**：同一行变成 **`unmet`**，notes 指明 **n=28 不足以让 95% 下界过 0.80**，
且 `chain_integrity` 那条**不变**。

🔴 **这一跑就是整个单的意义**：产品今天在打 `met`，而那个 `met` 我们已经同意站不住。
