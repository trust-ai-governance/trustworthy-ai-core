...
**G1 — `injection_catch_rate`** 🔴 **（2026-08-02 补全，承上游 B1/B2 核验）**

> `citation`：**可引，但不得表述为「通过双侧门 / 已达门槛」。** 89.29% 是**点估计**；
> `n=28` 的 Wilson 95% 区间为 **[72.8%, 96.3%]**，**下界低于 `τ_recall = 0.80`** ⇒
> 按 [EV-CIGATE](EV-CIGATE.md) 判据，该目标当前为 **`unmet`**。
> 🔴 **必带的防御性半句：「原因是 n 不足，不是拦截能力低」**（见 §1.1.1-B2）。
> **可引形式（逐字，必须抄进 citation）**：
> *「在 `<corpus_sha>` 这份 28 条注入语料上，拦截率 89.3%（n=28，95% CI 73%–96%）」*。
> **脆弱度**：**再漏 3 例即 22/28 = 78.6%，点估计亦跌破 0.80**（承 [EV-CIGATE](EV-CIGATE.md) §0 事实 2）。
> **达标声明须待 n≈150 且 CI 下界 ≥ 0.80。**

#### 1.1.1 🔴 两条核验裁定（B1 / B2）

**B1 —「脆弱度」那句：✅ bless 保留，但溯源改到 Core。**
算术我复核过（22/28 = 78.57%）；**而且它本来就是 Core 写的** ——
[EV-CIGATE](EV-CIGATE.md) §0 事实 2 逐字有「脆弱到 3 个 case」。
上游**不是自撰**，是把一条 **Core 已裁的事实**拉进了注；缺的是我在 §1.1 摘录时漏了它。
🔴 **判据（值得记成通则）：一句只会让主张【更窄】的注，不需要和放宽主张的注同等审查。**
**⇒ 已补进上面的 G1，溯源 = EV-CIGATE §0-2，不记 Platform 自撰。**

**B2 —「可引例句 + 防御半句」：✅ 确认补进 citation。**
现 artifact citation 写的是**规则**（"必须随带 n=28 + Wilson CI + 未达标口径"），**没有例句**。
两者不可互替：

- **规则**告诉读者**要带什么**；🔴 **例句告诉读者【做对了长什么样】** ——
  deck review 那次的 `0/14`，正是从"说明里的一条规则"提成"单元格里的一个数"才真正生效的。
- 🔴 **而「原因是 n 不足，不是拦截能力低」这半句更要紧：它防的是【反向误读】。**
  没有它，"未达标"会被读成"拦不住" —— **那和事实正好相反。**
  我们整轮在防数字被**高估**；这一句防的是同一个数字被**低估成另一个假话**。

**⇒ 一次幂等 re-annotate 补上（`lineage` 仍 `before == after`，值未动）。**
...
>
> 🔴 `citation`（**2026-08-02 收进本节，承 B3；措辞采上游草稿，逐字不改**）：
> **可对外引用为【本 run 规范来源】。必须随带：普查（非抽样、不适用 CI、
> 可由第三方用冻结段字节独立复验）+ 残余为覆盖面（其他/未来段未验）。
> 🔴 禁止写任何置信区间/下界（如「Wilson 下界 97.8%」）—— 普查无抽样不确定性，
> 区间反而暗示「最多 2.2% 记录可能坏」，而我们逐条验过、零条坏。**

**🆕 G4 — `unclosed_loop_rate`**（**2026-08-02 新增**；其权威是 [EV-CIGATE §1.5.1](EV-CIGATE.md) 的两轴）

> `notes`：**【估计轴】普查** —— 窗口内每个请求环逐个查过 ⇒ **CI 不适用**（与 `chain_integrity` 同构）。
> **【泛化轴】** 🔴 「0% 未闭环」是**这批探针语料**上测的；高负载/崩溃会变 ⇒
> **不得写成「本系统未闭环率 0%」**；且 **0% 同时是检测下界**（从未写入的记录不留缺口，无从发现）。
>
> `citation`（同上，措辞采上游草稿）：**可对外引用为【本 run 规范来源】。必须随带：
> 普查（不适用 CI）+ 语料口径（这批探针语料，非代表性流量）+ 检测下界。
> 🔴 禁止裸引「0%」/「无未闭环」。**

#### 1.1.2 🔴 B3 裁定：**这两条 citation 收进本节，溯源指向 Core**

上游问：由 Core 逐字收进 §1.1（溯源指 Core），还是认可「Platform 撰、语义据 Core」？

**⇒ 收进本节。理由不是署名，是【规则要有家】：**

1. 🔴 **这两条 citation 编码的是【裁定】，不是描述。**「禁止写任何置信区间」是一条**有 owner 的规则** ——
   而它的权威在 [EV-CIGATE §1.5 / §1.5.1](EV-CIGATE.md)（Core）。
   **一条规则的文本若只活在非 owner 手里，下次有人问「这里为什么不能写 CI」，
   线索会指向一份注，而不是指向那条判据。**
2. 🔴 **这正是我们刚为阈值建门要防的同一件事**（[GATE-CONSISTENCY](GATE-CONSISTENCY.md) 件二：
   **规则必须有机器可解析的权威落点，不能只活在散文里**）。用在字段上，同理。
3. **成本一次编辑。**

**措辞我逐字采用上游草稿，不重写** —— 🔴 **争的是它住在哪，不是谁写得更好；
上游那两段把「禁写什么」写成了可执行的句子，比我原稿更具体，值得原样保留。**

### 1.2 ✅ 答上游：**`canonical_source` 标不改**（2026-08-02，Core 作为 §5.0 仲裁表 owner 裁）

上游问：`injection_catch_rate` 被 [EV-CIGATE](EV-CIGATE.md) 判成 `unmet`、
`tool_scope_violation_rate` 的"零违规"也不能裸讲 —— **那它俩的 `canonical_source` 标要不要改？**

**🔴 不改。实读两份 artifact 就能定，不需要讨论：**

| indicator | `canonical_injection_run.json` | `canonical_chain_integrity.json` |
|---|---|---|
| `block_rate` · `boundary_breach_rate` · `chain_integrity` · `duration_p99` · `pii_exposure_surface` · `redaction_hit_ratio` · `terminal_error_ratio` · `unclosed_loop_rate`（**8 条**） | **`false`** | **`true`** |

**同一个指标，在一份里 `false`、在另一份里 `true`。**
⇒ 🔴 **`canonical_source` 回答的是"【哪一份产物】对这个指标说了算"，是一个【仲裁 / 去重】标 ——
它从来就不回答"这个数能不能支撑某个主张"。**
禁引那 8 条的 citation 也是这么写的：*"同 run 附带，不得对外引用 —— **规范来源见 §5.0 仲裁表**"*
—— **说的是"别引这一份拷贝，去引规范那份"，不是"这个数不可靠"。**

**⇒ 两根轴，别合并（又一次同族问题）：**

| 字段 | 回答 | 本例 |
|---|---|---|
| **`canonical_source`** | **哪一份产物说了算** | `injection_catch_rate` 的权威来源**就是**这份 injection artifact ⇒ **仍是 `true`** |
| **`citation`** | **这个数能说什么、不能说什么** | "可引，但不得表述为通过双侧门"（§1.1 G1） |

**若强行改 `canonical_source`，会得到两个更糟的结果：**

1. 🔴 **翻成 `false` = 这个指标【没有任何权威来源】** —— 那么任何人引 89.29% 都**没有产物可指**，
   从"权威但有限"退化成"无锚"。**更糟，不是更好。**
2. 🔴 **它会变成一个恒为 false 的字段**：按"不能支撑对外主张就翻 false"的标准，
   今天**所有**数都过不了（光流量口径一条就全挡）⇒ **标失去信息量。**

**⇒ 处置：`canonical_source` 一个字节不动**（因此**不需要**上游担心的那条独立 lineage）；
**全部约束由 `citation` 承载**（§1.1 三条已交付）。
🔴 **建议把这两句写进 §5.0 仲裁表的字段定义**，免得下一个人再来一遍。
